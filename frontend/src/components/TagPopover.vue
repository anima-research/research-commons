<template>
  <div
    v-if="show"
    class="tag-popover absolute z-50 mt-1 w-56 bg-white dark:bg-gray-800 rounded-md shadow-xl border border-gray-200 dark:border-gray-700 transition-colors text-xs"
    :style="{ left: `${position.x}px`, top: `${position.y}px` }"
    @click.stop
  >
    <div class="flex items-center justify-between px-2 py-1.5 border-b border-gray-200 dark:border-gray-700">
      <input
        v-model="searchQuery"
        type="text"
        placeholder="Search tags..."
        class="w-full px-2 py-1 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-[11px] text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 transition-colors"
        autofocus
      />
      <button
    ...
    <div class="max-h-64 overflow-y-auto py-1">
      <div
        ...
        class="border-b border-gray-100/70 dark:border-gray-700/70 last:border-0"
      >
        <!-- Ontology Header -->
        <div class="px-2 py-1 bg-gray-50 dark:bg-gray-900/40 text-[10px] font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-400 transition-colors">
          {{ onto.ontology.name }}
        </div>
        ...
        <div class="px-1.5 py-1 flex flex-col gap-1">
          <button
            ...
            class="w-full px-2.5 py-1.5 rounded flex items-center justify-between text-left hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            :class="{
